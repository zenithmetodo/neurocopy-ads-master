...
- `00-avatar-resumen/research.md` (SI existe: anuncios validados, ganchos, **DURACIONES típicas del nicho**, ofertas y voz del cliente · de `ads-research`). Si no existe, recomiendo correr **`ads-research`** antes de fabricar (la pesquisa es el 80%).
...
- **ads-research** → la pesquisa real (Meta Ad Library + voz del cliente) que aporta ángulos validados, ganchos que ya funcionan, DURACIONES típicas y ofertas. Lo consulto/recomiendo AL PRINCIPIO (si no hay `research.md`).
...
### Paso 5 · Validación ANTES de presentar (duraciones + 5 preguntas)
Antes del handoff, por CADA anuncio: digo su **DURACIÓN** (objetivo + estimada por los timestamps; si no cuadra, ajusto) y confirmo que pasó **LAS 5 PREGUNTAS DE TODO CUERPO** (amplifica el problema del gancho · mecanismo único · pruebas · fluye gancho→CTA · anticipa y elimina objeciones · ver `${CLAUDE_PLUGIN_ROOT}/knowledge/copy/biblia-del-copy.md` y `guion-body-master` Paso 7). Si alguno falla, lo corrijo antes de presentar.
...
11. **Pesquisa primero:** si no hay `research.md`, recomiendo correr `ads-research` (Ads Library + voz del cliente) antes de fabricar.
12. **Mecanismo SIEMPRE** (problema + solución + causa raíz como mínimo) en MEDIO/ALTO; nunca un anuncio MEDIO/ALTO sin mecanismo.
13. **Antes de presentar, valido cada anuncio:** digo su DURACIÓN y confirmo las 5 PREGUNTAS DE TODO CUERPO; si falla, corrijo.

---

## 🧠 El mecanismo en tu pieza (SIEMPRE se trabaja)

**Siempre** se trabaja, como mínimo, el **mecanismo del problema** (el villano / causa raíz: por qué el avatar tiene el problema) y el **mecanismo de la solución** (por qué tu método lo resuelve): son el motor de la credibilidad en los anuncios de nivel MEDIO y ALTO.
- Si el usuario trae su **OFERTA ya construida** (de `zenith-crea-ofertas`: promesa, mecanismo del problema/solución, nombre chicle, causa raíz, objeto brillante, mito de origen), úsala tal cual.
- Si **NO** la tiene, deduce y propón un mecanismo del problema + solución (y, si encaja, causa raíz + nombre chicle) coherente con el ángulo, **sin inventar cifras**, y déjalo marcado para que el usuario lo valide.
- **Nunca** escribas un anuncio de nivel MEDIO/ALTO sin mecanismo (en BAJO se despierta el problema; el mecanismo entra al subir de nivel).
